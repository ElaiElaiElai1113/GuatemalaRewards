...
function mapGiftCard(row: Record<string, unknown>): GiftCard {
  ...
  return {
    ...
    originalValueAmount: Number(mapped.originalValueAmount ?? 0),
    remainingValueAmount: Number(mapped.remainingValueAmount ?? 0),
    valueCurrency: (mapped.valueCurrency as string | null) ?? 'USD',
...
function publicGiftCardToGiftCard(card: PublicGiftCard): GiftCard {
  return {
    ...
    originalValueAmount: card.originalValueAmount,
    remainingValueAmount: card.remainingValueAmount,
    valueCurrency: card.valueCurrency,
...
function mapBusinessGiftCard(row: Record<string, unknown>): GiftCard {
  ...
  return {
    ...
    redemptionRemainingBalance: (mapped.redemptionRemainingBalance as number | null) ?? null,
